...
class TestProfile(TestCase):
    def setUp(self):
        self.user = User(id=1, username='rustique', password='uwimpaye12')
        self.user.save()
    #  def setUp(self):
    #         self.user = User(username='rustique')
    #     self.user.save()
...
class PostTest(TestCase):
    def setUp(self):
        self.user = User.objects.create(id=1, username='rustique')
...
class RatingTest(TestCase):
    def setUp(self):
        ...
        self.rating = Rating.objects.create(
            id=1, design=6, usability=7, content=9, user=self.user, post=self.post)
